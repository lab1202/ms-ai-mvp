...
- azure 구독중이고 사용하려는 툴은 langgraph, streamlit, azure search (index, indexer), azure openai, 미세조정, 컨텐츠 필터 등 교육자료에 나와있는 툴들이고 완성도를 높이기 위해 추가적으로 필요한 툴이 있으면 추가하여 사용가능하다.
...
현재까지 진행된 교육 자료는 첨부했고, 첨부된 파일에 있는 내용들을 참고해서 vscode로 어떻게 개방하면 좋을 지 작업해야하는 순서를 azure환경에서 리소스 생성부터 자세히 알려주고 저 시스템을 구현하기 위한 소스코드, 소스코드 설명, 로그 데이터 생성해주고 vscode 프로젝트로 다운로드 받을 수 있게 해줘
...
JSON으로 생성 후 테스트 데이터 항목 추가 필요하여 삭제후 재생성 시 오류 발생
 → 조교님 도움으로 해결.
